...
| [`docs/problems/PROBLEMS.md`](docs/problems/PROBLEMS.md) | Первичная карта проблем пользователя и связей с принципами и практиками |
| [`docs/methodology/README.md`](docs/methodology/README.md) | Правила развития методологии от проблемы к источнику |
...
Методология и контент строятся по цепочке:

> **Проблема пользователя → принцип → практика → контент → источник.**

Книга или существующая функция может подсказать гипотезу, но не определяет направление продукта без подтверждённой пользовательской проблемы.
