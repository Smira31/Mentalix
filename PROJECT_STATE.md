...
в `main` до начала этой сессии; этот раздел не был обновлён при закрытии,
расхождение зафиксировано этой правкой (тот же паттерн, что и
`MXL-UI-LAB-SHOWCASE-001` выше). cleanup сохраняет state и не уведомляет
Telegram, пока удаление не подтверждено HTTP 404/410 или Vercel inspect.
**Наблюдение 25.08.2026:** при живом закрытии Preview для gate
...
подтвердили удаление. Похоже на race между CLI-выводом об удалении и
фактическим распространением на edge — сам защитный механизм (не считать
удалённым без независимого подтверждения) сработал верно, но при
однократном retry внутри скрипта делает cleanup ненадёжным без ручного
вмешательства; возможный кандидат в `TASKS.md` → «Следующие задачи» отдельным
пунктом, если повторится.
