...
- `MXL-JOURNAL-MARKDOWN-001` реализована локально 25.08.2026 в ветке
  `codex/journal-markdown`: после обратной связи владельца одиночные записи
  ...
  менялись; unit/lint/build и UX smoke 20/20 зелёные. Предыдущий Preview
  подтверждённо удалён; новый целевой Preview опубликован и его
  `/api/health` вернул HTTP 200. Владелец принял визуал и работу редактора на
  реальном iPhone внутри Telegram 25.08.2026. Commit, PR и production пока не
  выполнены. Единый паттерн зафиксирован в `DESIGN_SYSTEM.md`; перенос остальных
  подходящих сценариев вынесен в `MXL-WRITING-CANVAS-ROLLOUT-001` и не входит в
  текущий diff.
...
**Текущая P0-задача — `MXL-JOURNAL-MARKDOWN-001`.** Реализация подготовлена
локально в отдельной ветке; целевой Preview и визуальная проверка с клавиатурой
на реальном iPhone пройдены владельцем 25.08.2026. Следующая граница — отдельные
commit и PR. До review/merge задача не является production-состоянием.
