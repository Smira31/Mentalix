...
test('MXL-JOURNAL-001 открывает Journal Flow из «Практик» и не подменяет им вкладку «Наставник»', () => {
  ...
  assert.match(journal, /verticallyCentered/)
  assert.doesNotMatch(journal, /<button[\s\S]*>\s*Назад\s*<\/button>/)
  assert.match(practices, /function JournalEntry\(/)
  assert.match(practices, /aria-label="Журнал"/)
...
test('MXL-HOME-QUIET-FOUNDATION-001 ставит главный Today hero перед вторичными секциями', () => {
  ...
  assert.match(today, /todayState === 'reviewPending' \? 'Разобрать день' : 'Начать чек-ин'/)
  assert.match(today, /Сегодняшний цикл завершён/)
  assert.doesNotMatch(today, /Открыть разбор снова/)
