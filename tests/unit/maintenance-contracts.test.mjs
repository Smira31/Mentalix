...
test('MXL-JOURNAL-001 открывает единственный Guided Self-Discovery Journal flow из «Практик»', () => {
  ...
  assert.match(journal, /PracticeWritingCanvas/)
  assert.match(journal, /useFullscreenSurface/)
  assert.match(journal, /JournalArt/)
...
test('MXL-016 публикует семь авторских мыслей без непроверенной атрибуции', () => {
  ...
  assert.match(today, /const \[thoughtOfDay\] = useState\(\(\) => getDailyThought\(\)\)/)
  assert.doesNotMatch(today, /dailyQuote/)
...
test('MXL-HOME-QUIET-FOUNDATION-001 публикует одну главную карточку дня', () => {
  ...
  assert.match(todayStyles, /\.mx-today-day-card\s*\{[\s\S]*height:\s*455px/)
  assert.match(todayStyles, /\.mx-today-day-card\[data-complete='true'\]\s*\{[\s\S]*height:\s*233px/)
