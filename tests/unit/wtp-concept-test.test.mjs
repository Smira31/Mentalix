...
test('MXL-WTP-001 compares three paid outcome concepts without checkout or backend', () => {
  assert.match(flow, /Разбор одной ситуации/)
  assert.match(flow, /Обзор повторений/)
  assert.match(flow, /Глубокий разбор с ИИ/)
...
test('MXL-WTP-001 is reachable from Settings and does not alter the Today route', () => {
  ...
  assert.match(settings, /Короткий опрос — без оплаты и подписки/)
