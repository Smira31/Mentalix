...
test('локальный UX smoke по основному маршруту', async ({ browser, baseURL }) => {
  ...
  for (const viewport of VIEWPORTS) {
    ...
    await captureScreen({
      ...
      results,
      check: async () => {
        const checkin = page.getByRole('button', { name: /Начать|Пройти чек-ин|Разобрать день/ })
        await assertClickable(checkin)
      },
    })

    await page.getByRole('button', { name: /Начать|Пройти чек-ин|Разобрать день/ }).click()
    await captureScreen({
      page,
      viewport,
      screen: 'Check-in',
