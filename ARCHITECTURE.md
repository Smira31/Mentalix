...
5. API-клиент уже нормализует HTTP/network/protocol/timeout ошибки, применяет timeout 10 секунд и один retry только для безопасных idempotent-запросов. Управляемой отмены со стороны экранов, telemetry и единого отображения ошибок пока нет.
6. Query-параметры API централизованно сериализуются через
   `src/lib/apiQuery.js::withQuery` (`URLSearchParams`).
7. `Today` не кэширует и не маскирует ошибку критичных данных как empty-state; состояния loading/error/empty на остальных экранах по-прежнему реализуются неравномерно.
...
Отступы сверху и снизу принадлежат `App.jsx`. Горизонтальный отступ экрана задаётся
одним токеном `--mx-screen-x: 16px`; обёртки экранов используют его, а не `px-*`
классы. До этого
