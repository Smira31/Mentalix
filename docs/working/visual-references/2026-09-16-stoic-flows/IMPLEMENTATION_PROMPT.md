...
Отдельное исключение: иконки реакций (Нет/Немного/Да) используют thumbs/hand-графику (lucide-react ThumbsDown/Hand/ThumbsUp) по отдельному референсу владельца, а не круглые лица из 05-checkin-completion-after.png — размер и геометрия карточек при этом берутся из committed PNG.
