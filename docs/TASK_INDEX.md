...
| `MXL-JOURNAL-001`             |      ML | product/UX              | completed   | verified                              | Journal Home prototype вошёл в main через PR #224, merge commit `ab76e7a`; ручной gate пройден                    |
...
| `MXL-JOURNAL-PERSISTENCE-001` |      XL | product/backend         | completed   | local-first complete; cloud-dependent | PR #232 смёржен в main (`26adf72`); draft/final и legacy migration готовы, cloud sync не реализован               |
| `MXL-JOURNAL-HISTORY-001`     |   S–M/L | product/backend/UX      | in progress | local read-only now; cloud-dependent  | Read-only adapter сегодняшней local-first записи в History; текущая ветка `feat/mxl-journal-history-readonly-001` |
| `MXL-JOURNAL-PRIVACY-001`     |       L | product/safety/backend  | needs-owner | legal/safety + backend-dependent      | До cloud/AI решить consent, retention, provider disclosure, export/delete и telemetry boundary                    |
