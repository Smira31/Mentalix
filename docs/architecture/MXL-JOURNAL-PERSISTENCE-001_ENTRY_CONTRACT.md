...
**Дата:** 29.08.2026, GMT+3. **Исправлено:** 29.08.2026 (addendum ниже).
...
> **Addendum 29.08.2026 (исправление фактической ошибки в §2):** первоначальный аудит утверждал, что четыре one-shot practice-flow не сохраняют факт завершения вообще. Это неверно и исправлено ниже. `src/lib/firstStepPractice.js` (PR #61, `MXL-PRB-002`/`MXL-DEC-013`), `src/lib/noBlamePractice.js` (PR #61, `MXL-PRB-001`/`MXL-DEC-014`), `src/lib/oneFinishPractice.js` (PR #62, `MXL-PRB-003`/`MXL-DEC-015`) и `src/lib/narrowFocusPractice.js` (PR #63, `MXL-PRB-015`/`MXL-DEC-016`) уже реализуют local-only лог завершений (`readLocal`/`writeLocal`, `RETAIN_ENTRIES=60`, запись формы `{day, outcome, reflection, completed_at}`) и реально вызываются из соответствующих flow-компонентов при completion (`saveFirstStepEntry`/`saveNoBlameEntry`/`saveOneFinishEntry`/`saveNarrowFocusEntry`). Первоначальный аудит прочитал эти flow-компоненты, но не заметил вызовы `saveXxxEntry` и не проверил их импорты в `src/lib/`. Ни `docs/TASK_INDEX.md`, ни `TASKS.md` этот существующий persistence не фиксируют. Это меняет §2, §9 и §10 ниже (правки внесены inline, зачёркнутый вывод не удалён — оставлен для истории решения).
...
>
> **Главный вывод (исправлено):** «завершённая практика» сейчас означает **четыре** разных, не связанных между собой вещи в коде, и все четыре уже что-то сохраняют — но по четырём несовместимым схемам. Journal Flow имеет полноценное local-first хранилище с версионированием и историей (`journalStorage.js`). Check-in (`lessons`/`wins`) сохраняется отдельно через backend. Четыре one-shot practice-flow каждый пишут в свой собственный local-only лог (`firstStepPractice.js`/`noBlamePractice.js`/`oneFinishPractice.js`/`narrowFocusPractice.js`) с общей структурой записи, но без `entry_id`, без `schema_version`, без idempotency-контракта и без единого read-model — четыре параллельные копии одного паттерна, не один пробел в персистентности. Главный пробел — не отсутствие сохранения, а отсутствие единого entry contract и History UI поверх всех четырёх логов.
...
## 2. Что уже реализовано локально (аудит кода) — исправлено 29.08.2026
...
| **Одноразовые practice-flow** (`FinishFlow`→`oneFinishPractice.js`, `NarrowFocusFlow`→`narrowFocusPractice.js`, `ProcrastinationFlow`→`noBlamePractice.js`, `FirstStepFlow`→`firstStepPractice.js`) | `localStorage`, `mx-{first-step\|narrow-focus\|no-blame\|one-finish}-v1:<userId>`, четыре отдельных файла в `src/lib/`, вызываются из `onComplete` каждого flow | **Уже персистентно.** Каждый пишет запись `{day, outcome, reflection, completed_at}` через свой `saveXxxEntry(userId, {outcome, reflection})`, читает через `readXxxLog(userId)`, хранит последние 60 (`RETAIN_ENTRIES`) записей на практику | Нет `entry_id`, нет `schema_version`, нет idempotency по id (upsert невозможен — каждый вызов просто добавляет запись), нет общего read-model/History UI поверх всех четырёх, дублирование одного паттерна в четырёх файлах |

Из этого следует, что «practice type» в контракте ниже (§4) должен явно перечислить домены, а не считаться самоочевидным — «Journal», «check-in reflection» и «one-shot practice completion» это разные вещи с разным текущим состоянием готовности; последнее уже local-first персистентно, но не унифицировано.
...
Journal Flow пишет по дате (upsert по ключу), check-in — отдельный, уже проверенный контракт. Домен `one_shot_practice` (исправлено §2: он уже пишет, но без idempotency) — это подтверждённый, не гипотетический риск: текущие `saveXxxEntry` в четырёх файлах-логах ничего не upsert'ят — каждый вызов `[...log, entry].slice(-RETAIN_ENTRIES)` просто добавляет новую запись, так что повторный `onComplete` (двойной тап, повторный рендер в StrictMode — см. существующий инвариант «Побочные эффекты» в `AI_RULES.md` §9 про двойной вызов апдейтера) создаст два разных завершения на один и тот же flow-запуск, если flow-компонент сам не защищён (эти файлы не проверялись на этот счёт — flow-компоненты вне ownership этой задачи). Предложение остаётся тем же: `entry_id` генерируется клиентом один раз при входе в flow (не при завершении), передаётся неизменным через весь flow, upsert по `entry_id`, а не insert — это применимо и к будущему backend, и к любому local-first адаптеру, который переносил бы эти четыре лога на единый контракт.
...
## 10. Что можно сделать frontend-only, без backend, прямо сейчас — исправлено 29.08.2026

Явно НЕ реализовано в рамках этой задачи (docs-only). Исходная версия этого раздела предлагала «завести local-only completion-record для четырёх one-shot practice-flow» как next step — это неверно (§2 addendum): такой record уже существует, четырежды, независимо друг от друга. Реальный следующий шаг — не создание, а **унификация**: новый read/write-адаптер в `src/lib/`, который умеет читать существующие четыре лога (`firstStepPractice.js`/`noBlamePractice.js`/`oneFinishPractice.js`/`narrowFocusPractice.js`) как есть (не переписывая их — они принадлежат зоне action loop/practices, не этой задаче) и предоставлять их через единый entry contract (§4: `entry_id`, `schema_version`, `practice_type=one_shot_practice`) для будущего History UI, не трогая сами flow-компоненты или их storage-файлы. Подключение этого адаптера к записи (чтобы новые completion сразу писались по новому контракту, включая idempotent upsert по `entry_id`) требует правки `saveXxxEntry`-вызовов внутри flow-компонентов — это вне ownership этой задачи и зоны другого агента (action loop/practices).
