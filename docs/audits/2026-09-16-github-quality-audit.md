...
| P2 | Disabled workflow files | `release.yml`, `pr-handoff.yml`, `issue-triage.yml` and `telegram-owner-qa-webapp.yml` remain in the tree but are `disabled_manually`. This makes code inventory look more active than GitHub runtime state. | Keep them disabled; decide whether to move their historical rationale to archive and delete workflow files in a dedicated PR. | Medium: re-enabling/removing may affect established operations. | Owner decision |
...
Manually disabled workflows: `Create release`, `PR handoff report`, `Issue triage`, `Telegram Owner QA Web App`. Historical runs may still appear in `gh run list`; they do not mean the workflow is currently enabled.
