...
## 20.09.2026 — MXL-BRAND-SYMBOL-BOOK-QUILL: книга с пером вместо лабиринта

- Фирменный символ Mentalix заменён на статичную монохромную SVG-иконку
  раскрытой книги с пером (`src/components/BookLogo.jsx`). Она использует
  `currentColor`, работает в обеих темах и не принимает `progress`.
- Обновлены splash, экран AppLock, центральная вкладка «Диалог», UI Lab,
  `public/favicon.svg` и четыре PWA-иконки. Прогресс-логика и анимация
  старого символа полностью удалены; прогресс-точки onboarding и текстовая
  подпись оставшихся действий не менялись.
- Явно отменено «Обновление 10 — Лабиринт как символ + полный экран» как
  устаревшее продуктовое решение. Историческая запись сохранена ниже.
- Проверки: `grep` по `MazeLogo` в `src/` и `public/`, `npm run lint`,
  `npm run build`, `git diff --check`.

## 20.09.2026 — MXL-STREAK-CHECKIN-001: экран классической серии после утреннего чек-ина

- Экран завершения утреннего чек-ина получил шести-лепестковую иконку на токенах темы, русскую подпись без мотивационных клише и CTA «Вернуться в Сегодня».
- Исправлен фронтовый кейс, когда после успешного сохранения показывался `0`: UI гарантирует минимум однодневную серию, не меняя backend-расчёт.
- Фиксированная неделя Пн–Вс заменена растущим рядом дней, заканчивающимся текущим днём; в шапке streak-экрана оставлена только кнопка закрытия.
- Прежнее анти-streak решение отменено владельцем 20.09.2026; актуальная формулировка и сохранённая история решения находятся в `STOIC_FEATURES.md`.
...
- Флоу: карта/дилемма → 1–3 вопроса → AI формирует гипотезу → пользователь подтверждает → гипотеза переводится в существующий core loop. После сессии — профиль тем через `useSynced`, влияет на подборку в Explore/Практиках («Все практики» остаётся доступно).
- Issue #434 создан с тем же лейблом, что аналогичные idea-candidate записи (`🧠 Идея`) — не `docs`/`preview-only` от issue #416 (та идея уже прошла diagnostic-этап, Лила его ещё не начинала).
- **Зафиксирована, не в код-очереди.** Перед кодом обязателен pre-mortem + ADR в `docs/core/PRODUCT_DECISIONS.md` — решение принимает владелец.
...
## 31.08.2026 — MXL-FULLSCREEN-HEADER-NATIVE-001, часть 2: безопасный точечный перевод

- Запрошенная задача была сформулирована как «заменить кастомные X/back на нативный Telegram BackButton API», но существующий discovery/pre-mortem черновик (`TASKS.md`) по этой же задаче явно заключает: «массовая замена не нужна» и «не менять визуальный или навигационный контракт массово» до отдельного решения владельца о семантике ← vs X в `CheckIn`. Массовую замену делать не стала — задокументированное предостережение имеет приоритет над буквальной формулировкой задачи.
- Проверила по факту кода все 15 потребителей `FULLSCREEN_HEADER_SLOT_CLASS`: 11 уже используют общий `BackButton` (который сам вызывает `useBackButton()` — нативный Telegram API — внутри Telegram, и рисует кастомную кнопку только в браузере) — задача по факту почти выполнена ранее, отдельными изменениями.
- Единственный безопасный точечный кейс: `GuidedJournals.jsx::CompletedSessionViewer` — кнопка «← К архиву» была кастомной, без конкурирующего действия рядом. Переведена на общий `BackButton` (`label="К архиву"`).
- `CheckIn.jsx` (← = шаг назад в flow ИЛИ закрыть на step 0; X = всегда закрыть — разные действия) и `MoodCheckGate.jsx` («Пропустить» — это движение вперёд мимо интерстишла, не «назад») оставлены нетронутыми, как и требует черновик. `ThemeScreen.jsx` — заголовок вообще пустой (нет кнопки), это отдельный, не связанный с этой задачей пробел, не трогала.
...
- Один из трёх исходных P1-блокеров снят сам по себе: PR #312 (`feat: clarify direct web auth fallback`), который отчёт не смог проверить через Preview, уже смёржен в `main` 29.08.2026 — отдельного Preview для него больше не нужно, он проверяется как часть обычного `main`.
- Day rollover (шаг 9) отмечен как отдельный практический вопрос — за одну сессию без backend test-режима смены даты его не пройти; можно дозаполнить на следующий день, не блокируя остальные 9 шагов.
- **Не задеплоено:** Vercel free-tier quota на проекте `mentalix-preview` подтверждена ещё исчерпанной (rate limit, retry in 24h) при проверке CI несколько минут назад — деплой намеренно не запускался повторно, чтобы не тратить попытку впустую. Preview будет отправлен в Telegram, как только квота восстановится.
- Issue #356 не закрыт — это полноценный аутентифицированный цикл с реальным аккаунтом, требует владельца.

## 31.08.2026 — MXL-JOURNAL-001: gate пройден, PASS (#355, PR #422)

- Владелец прошёл чек-лист `docs/qa/MXL-JOURNAL-001_TELEGRAM_IPHONE_GATE.md` на реальном iPhone в Telegram, вердикт **PASS** — зафиксировано в разделе 8 файла и в issue #355. Issue закрыт.
- Пошаговые результаты по разделам 1–7 не были продиктованы построчно (устройство/iOS/скриншоты не приложены) — общий вердикт PASS зафиксирован со слов владельца. Файл явно помечает это ограничение записи; для release-документации при необходимости можно дозаполнить позже, релиз это не блокирует.
...
**Исправление 31.08.2026 (позже в этот же день):** утверждение выше о `VITE_STARTER_SET_ENABLED=false` оказалось неверным — squash-merge PR #419 (`ae2cc714`, 12:37 МСК) обогнал env-gate фикс (`6891674c`, 12:46 МСК) на 9 минут и не включил его. `main` фактически ship'ил `STARTER_SET_ENABLED = true`, статично, без гейта, всем пользователям. См. запись «MXL-STARTER-SET-001 v1 закрыт» выше — там же решение владельца оставить как есть. Комментарий в issue #417 обновлён с исправлением.
...
- Ручная проверка на iPhone 16 Pro Max в Telegram (30.08.2026), все 5
  кейсов — PASS: UX-R-01 (закрытие/повторный запуск), UX-R-09 (длинное
  сообщение AI), UX-R-10 (закрытие жестом и кнопкой), UX-R-14 (быстрое
  переключение вкладок), UX-R-16 (полный Check-in с сохранением).
  Замечаний нет.
- В CI (GitHub Actions) на этом же PR Playwright-смоук прошёл зелёным —
  подтверждает, что локальный флейки-таймаут теста «локальный UX smoke по
  основному маршруту» (см. запись ниже) был локальной ресурсной проблемой
  (машина была занята обычным браузингом), а не регрессией от изменения.
- [PR #347](https://github.com/Smira31/Mentalix/pull/347) squash-merge в
  `main` коммитом `00814da4`. Ветка
  `claude/mxl-ux-responsive-001-writing-goal-rollback` удалена (remote и
  local).
- `TASKS.md`: добавлен подэтап «Закрытый подэтап (30.08.2026)» в записи
  `MXL-UX-RESPONSIVE-001` — общий статус задачи (`ready / owner-priority`
  для v1.1.0) не менялся, так как это узкий частный фикс, а не закрытие
  всей umbrella-задачи responsive UI.

## 30.08.2026 — MXL-UX-RESPONSIVE-001: rollback цели записи при ошибке сохранения

- `src/screens/Settings.jsx`, `saveWritingGoal`: оптимистичное обновление
  `writingGoalOn`/`writingGoalCount` теперь откатывается при ошибке
  сохранения. До правки `catch` только показывал сообщение об ошибке, а UI
  оставался в оптимистичном (несохранённом) состоянии — расхождение с тем,
  что реально лежит на backend. Добавлены `previousEnabled`/`previousCount`
  перед оптимистичным `setState`; в `catch` состояние возвращается к ним, а
  текст статуса уточнён: «Не удалось сохранить цель записи. Настройка
  возвращена без изменений.» Изменена только эта функция; `src/lib/api.js`,
  backend и другие экраны не менялись.
- `npm run check:core` — 146/146 unit, lint, build, docs:check — PASS.
- `npm run ux:check` — 3/4 PASS. Один тест
  (`локальный UX smoke по основному маршруту`) стабильно падает по
  таймауту (300s) на клике `Практики` после `Назад` — на шаге, который не
  затрагивает Settings/`saveWritingGoal` (этот сценарий не открывает экран
  настроек). Разово прогнан на чистом `origin/main` в отдельном worktree —
  там тест прошёл (2.2m), но на текущей ветке падал воспроизводимо трижды
  подряд, включая изолированный прогон без параллельной активности. На
  машине в это время было аномально много процессов `chrome.exe` от
  обычного использования браузера пользователем, что может объяснять
  системную медленность/таймаут независимо от кода. Причинно-следственная
  связь с этим диффом не подтверждена (изменённая функция не участвует в
  проверенном сценарии), но и не опровергнута с полной уверенностью —
  честно фиксирую как нерешённое расхождение, а не как «зелёный» результат.
- Ветка `claude/mxl-ux-responsive-001-writing-goal-rollback` создана от
  свежего `origin/main`. Изменения застейджены, не закоммичены и не
  запушены — ждут ручной проверки diff владельцем.
...
- Раздел «Language» в `AGENTS.md` уже требовал отвечать пользователю
  по-русски, но по прямому запросу владельца добавлена явная строка на
  русском языке, чтобы правило не терялось между сессиями/агентами:
  «Отвечай пользователю только по-русски».
- Продуктовый код не менялся. `git diff --check`, `npm run docs:check` —
  PASS. PR, squash-merge `docs/agents-language-rule-ru` → `main`.
...
- PR #287 смёржен squash-коммитом `47d62eba` в `main`, ветка удалена.
  Preview проверен и одобрен владельцем на iPhone. Подробности —
  `TASKS.md`.

## 28.08.2026 — MXL-DS-LABEL-FONT-001: вторичный шрифт Manrope для лейблов

- Manrope (500/600, самостоятельный хостинг через `@fontsource/manrope`,
  без Google Fonts CDN) подключён как новый токен `font-label` в
  `tailwind.config.js` и применён к 27 эйбраузам/caps-лейблам в 16
  файлах над заголовками карточек и в секционных подписях, включая
  «Наблюдения»/«Цифры»/«Данные» на Analytics; `display`/`body` (Onest)
  не изменились. Контрактный тест `MXL-TYPE-SYSTEM-001` обновлён:
  вместо буквального поиска слова «Manrope» проверяет канонический
  класс `font-label` и его границы (только 3 эйбрауза Analytics, не
  числа/графики/экран). Подробности и полный список файлов — `TASKS.md`,
  `MXL-DS-LABEL-FONT-001`.

## 28.08.2026 — MXL-JOURNAL-001: 4-фазный Journal Flow по видео-референсу

- По решению владельца выбран вариант А: текущим входом Journal становится отдельный локальный flow «Идея → Действие → Анализ → Новый шаг», а не семидневная тема.
- В Preview-ветке добавлены закреплённая верхняя карточка над разделом «Практики», вступление, понятные действия сохранения, центрированные intro/completion-сцены, удаление дублирующей кнопки «Назад» и повторное открытие сохранённой записи.
- Документация и ограничения зафиксированы в `docs/research/journal_video_reference_2026-08-28.md`; backend/API, облачная синхронизация, данные, шестая вкладка и PR #268 не меняются.
- Локально проходят unit, lint, build, UX smoke на мобильных viewport и `git diff --check`; ручной Telegram/iPhone gate остаётся обязательным.
