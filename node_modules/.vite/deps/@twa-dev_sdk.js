...
//#region node_modules/@twa-dev/sdk/dist/telegram-web-apps.js
var require_telegram_web_apps = /* @__PURE__ */ __commonJSMin((() => {
	(function() {
		var eventHandlers = {};
		var locationHash = "";
		try {
		...
		function postEvent(eventType, callback, eventData) {
			...
			else callback({ notAvailable: true });
		}
		...
		function callEventCallbacks(eventType, func) {
			...
			if (curEventHandlers === void 0 || !curEventHandlers.length) return;
			for (var i = 0; i < curEventHandlers.length; i++) try {
				func(curEventHandlers[i]);
			} catch (e) {}
		}
		function onEvent(eventType, callback) {
			if (eventHandlers[eventType] === void 0) eventHandlers[eventType] = [];
			if (eventHandlers[eventType].indexOf(callback) === -1) eventHandlers[eventType].push(callback);
		}
		function offEvent(eventType, callback) {
			if (eventHandlers[eventType] === void 0) return;
	...
	(function() {
		...
		if (initParams.tgWebAppDefaultColors && initParams.tgWebAppDefaultColors.length) {
			...
			try {
				...
				if (def_colors) setDefaultColors(def_colors);
			} catch (e) {}
		}
		...
		if (initParams.tgWebAppFullscreen) setFullscreen(true);
		...
		if (stored_orientation_lock) setOrientationLock(stored_orientation_lock == "yes");
		...
		function onSafeAreaChanged(eventType, eventData) {
			if (eventData) setSafeAreaInset(eventData);
		}
		function onContentSafeAreaChanged(eventType, eventData) {
			if (eventData) setContentSafeAreaInset(eventData);
		}
		...
		function setDefaultColors(def_colors) {
			if (colorScheme == "dark") {
				if (def_colors.bg_dark_color) webAppBackgroundColor = def_colors.bg_dark_color;
				if (def_colors.header_dark_color) {
					webAppHeaderColorKey = null;
					webAppHeaderColor = def_colors.header_dark_color;
				}
			} else {
				if (def_colors.bg_color) webAppBackgroundColor = def_colors.bg_color;
		...
		function setViewportHeight(data) {
			...
			if (viewportHeight !== false) height = viewportHeight - bottomBarHeight + "px";
			else height = bottomBarHeight ? "calc(100vh - " + bottomBarHeight + "px)" : "100vh";
			if (viewportStableHeight !== false) stable_height = viewportStableHeight - bottomBarHeight + "px";
			else stable_height = bottomBarHeight ? "calc(100vh - " + bottomBarHeight + "px)" : "100vh";
		...
		function onHomeScreenChecked(eventType, eventData) {
			...
			receiveWebViewEvent("homeScreenChecked", { status });
		}
		...
		function onPopupClosed(eventType, eventData) {
			if (webAppPopupOpened) {
				...
				receiveWebViewEvent("popupClosed", { button_id });
			}
		}
		...
		function isColorDark(rgb) {
			...
			return Math.sqrt(.299 * (r * r) + .587 * (g * g) + .114 * (b * b)) < 120;
		}
		...
		if (initParams.tgWebAppDebug) {
			...
			for (var k in debugBottomBarStyle) debugBottomBar.style[k] = debugBottomBarStyle[k];
			...
			if (mainBtn.isVisible || secondaryBtn.isVisible) {
				...
				if (mainBtn.isVisible && secondaryBtn.isVisible) {
					if (secondaryBtn.position == "top") {
						...
						bottomBarHeight += 51;
					} else if (secondaryBtn.position == "left") debugBottomBar.style.flexDirection = "row-reverse";
					else if (secondaryBtn.position == "right") debugBottomBar.style.flexDirection = "row";
				}
			} else {
		...
		var BottomButtonConstructor = function(type) {
			...
			if (!isMainButton) Object.defineProperty(bottomButton, "position", {
				set: function(val) {
			...
			function onBottomButtonPressed() {
				if (isActive) receiveWebViewEvent(webViewEventName);
			}
			function buttonParams() {
				...
				} else var params = { is_visible: false };
				return params;
			}
			function buttonState(btn_params) {
				if (typeof btn_params === "undefined") btn_params = buttonParams();
			...
			function updateDebugButton(btn_params) {
				...
				} else debugBtn.style.display = "none";
			...
			function setParams(params) {
				...
				if (typeof params.color !== "undefined") if (params.color === false || params.color === null) buttonColor = false;
				else {
				...
				if (typeof params.text_color !== "undefined") if (params.text_color === false || params.text_color === null) buttonTextColor = false;
				else {
			...
			bottomButton.hideProgress = function() {
				if (!bottomButton.isActive) isActive = true;
		...
		var BiometricManager = (function() {
			...
			function onBiometryInfoReceived(eventType, eventData) {
				...
				if (accessRequestState) {
					...
					if (state.callback) state.callback(isAccessGranted);
				}
			...
			function onBiometryAuthRequested(eventType, eventData) {
				...
				if (authRequestState) {
					...
					if (state.callback) state.callback(isAuthenticated, isAuthenticated ? biometricToken : null);
				}
			...
			function onBiometryTokenUpdated(eventType, eventData) {
				...
				if (tokenRequestState) {
					...
					if (state.callback) state.callback(applied);
				}
		...
		var LocationManager = (function() {
			...
			function onLocationRequested(eventType, eventData) {
				if (!eventData.available) locationData = null;
					...
					if (typeof eventData.altitude !== "undefined" && eventData.altitude !== null) locationData.altitude = eventData.altitude;
				...
				} else locationResponse(locationData);
			}
		...
		var Accelerometer = (function() {
			...
			accelerometer.start = function(params, callback) {
				params = params || {};
				if (!checkVersion()) return accelerometer;
				var req_params = {};
				var refresh_rate = parseInt(params.refresh_rate || 1e3);
				if (isNaN(refresh_rate) || refresh_rate < 20 || refresh_rate > 1e3) console.warn("[Telegram.WebApp] Accelerometer refresh_rate is invalid", refresh_rate);
				else req_params.refresh_rate = refresh_rate;
		...
		var DeviceOrientation = (function() {
			...
			deviceOrientation.start = function(params, callback) {
				params = params || {};
				if (!checkVersion()) return deviceOrientation;
				var req_params = {};
				var refresh_rate = parseInt(params.refresh_rate || 1e3);
				if (isNaN(refresh_rate) || refresh_rate < 20 || refresh_rate > 1e3) console.warn("[Telegram.WebApp] DeviceOrientation refresh_rate is invalid", refresh_rate);
				else req_params.refresh_rate = refresh_rate;
		...
		var Gyroscope = (function() {
			...
			gyroscope.start = function(params, callback) {
				params = params || {};
				if (!checkVersion()) return gyroscope;
				var req_params = {};
				var refresh_rate = parseInt(params.refresh_rate || 1e3);
				if (isNaN(refresh_rate) || refresh_rate < 20 || refresh_rate > 1e3) console.warn("[Telegram.WebApp] Gyroscope refresh_rate is invalid", refresh_rate);
				else req_params.refresh_rate = refresh_rate;
				if (callback) startCallbacks.push(callback);
		...
		function onQrTextReceived(eventType, eventData) {
			if (webAppScanQrPopupOpened) {
				...
				receiveWebViewEvent("qrTextReceived", { data });
			}
		}
		...
		function onClipboardTextReceived(eventType, eventData) {
			if (eventData.req_id && webAppCallbacks[eventData.req_id]) {
				...
				receiveWebViewEvent("clipboardTextReceived", { data });
			}
		}
		...
		function onFileDownloadRequested(eventType, eventData) {
			if (webAppDownloadFileRequested) {
				...
				if (requestData.callback) requestData.callback(isDownloading);
		...
		function invokeCustomMethod(method, params, callback) {
			...
			webAppCallbacks[req_id] = { callback };
		...
		WebApp.showPopup = function(params, callback) {
			...
			webAppPopupOpened = { callback };
			WebView.postEvent("web_app_open_popup", false, popup_params);
		};
		WebApp.showAlert = function(message, callback) {
			WebApp.showPopup({ message }, callback ? function() {
				callback();
			} : null);
		};
		...
		WebApp.showScanQrPopup = function(params, callback) {
			...
			webAppScanQrPopupOpened = { callback };
		...
		WebApp.requestWriteAccess = function(callback) {
			...
			WebAppWriteAccessRequested = { callback };
		...
		WebApp.requestContact = function(callback) {
			...
			WebAppContactRequested = { callback };
		...
		WebApp.downloadFile = function(params, callback) {
			...
			webAppDownloadFileRequested = { callback };
		...
		WebApp.shareMessage = function(msg_id, callback) {
			...
			WebAppShareMessageOpened = { callback };
		...
		WebApp.setEmojiStatus = function(custom_emoji_id, params, callback) {
			...
			if (typeof params.duration !== "undefined") status_params.duration = params.duration;
			if (WebAppEmojiStatusRequested) {
				console.error("[Telegram.WebApp] Emoji status is already requested");
				throw Error("WebAppEmojiStatusRequested");
			}
			WebAppEmojiStatusRequested = { callback };
		...
		WebApp.requestEmojiStatusAccess = function(callback) {
			...
			WebAppEmojiStatusAccessRequested = { callback };
...
//#endregion
//#region node_modules/@twa-dev/sdk/dist/sdk.js
...
//#endregion
//#region node_modules/@twa-dev/sdk/dist/index.js
var require_dist = /* @__PURE__ */ __commonJSMin(((exports) => {
	Object.defineProperty(exports, "__esModule", { value: true });
	exports.default = require_sdk().WebApp;
}));
//#endregion
