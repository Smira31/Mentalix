...
import { readLastInsightDate } from './insightDigest'
import { insightIntervalElapsed, shouldShowSurprise } from './surpriseRules'

export async function maybeBuildSurprise(user, { random = Math.random, today = toLocalCalendarDate() } = {}) {
  try {
    ...
    const force = isPreviewDemoMode() && new URLSearchParams(window.location.search).get('surprise') === '1'
