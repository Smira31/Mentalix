...
export default function JournalStart({
  ...
  const meta = PERSONAS.find(item => item.key === persona)
  ...
  return (
    <div className="w-full max-w-md mx-auto px-5 flex flex-col min-h-[calc(100vh-160px)] animate-fade-in">
      ...
      <div className="flex items-center pt-1">
        ...
        <div className="ml-4 flex items-center gap-3 min-w-0">
          <div className="w-10 h-10 rounded-xl border border-gold/25 bg-gold/[0.04] flex items-center justify-center shrink-0">
            <Icon size={22} className="text-gold" strokeWidth={1.7} />
          </div>

          <div className="min-w-0">
            <div className="mx-ai-title text-cream leading-none">{meta.name}</div>

            <div className="mx-ai-meta text-gold mt-1">{meta.tagline}</div>
          </div>
        </div>
      ...
      <div className="pt-8">
        <h1 className="mx-ai-title text-cream whitespace-pre-line font-display">{meta.question}</h1>

        <div className="w-8 h-px bg-gold mt-7 mb-5" />

        <p className="mx-ai-body text-muted max-w-[300px]">{meta.intro}</p>
      </div>
