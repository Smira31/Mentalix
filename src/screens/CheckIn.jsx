...
import { pickByDay, JOURNAL_MODES, LESSON_PROMPTS } from '../data/prompts'
...
export default function CheckIn({ user, onDone, mode = 'checkin', existing = null }) {
  ...
  const [journalMode, setJournalMode] = useState('brief')

  const [journalQuestions] = useState(() =>
    Object.fromEntries(
      Object.entries(JOURNAL_MODES).map(([key, mode]) => [key, pickByDay(mode.questions)])
    )
  )
  ...
  const journalModeConfig = JOURNAL_MODES[journalMode]
  const journalQuestion = journalQuestions[journalMode]

  const questionTitle = isMorningNoteStep
    ? journalQuestion
    : scale?.title ||
      (isEmotionStep
        ? 'Что ближе всего?'
  ...
  const questionSubtitle = isMorningNoteStep
    ? journalModeConfig.description
    : scale?.hint ||
      (isEmotionStep
          ...
          <section className={isMorningNoteStep ? 'w-full text-left' : CHECKIN_QUESTION_CLASS}>
            ...
            <h2
              className={[
                ...
                isMorningNoteStep ? 'text-[24px] leading-[1.12]' : 'text-[22px] leading-tight',
              ].join(' ')}
            >
              {questionTitle}
            </h2>
                ...
                {isEvening ? (
                  <div className="space-y-3 max-w-md mx-auto w-full">
                    {LESSON_FIELDS.map(field => (
                      <div key={field.key} className="rounded-3xl bg-emerald p-4">
                        <div className="text-[12px] font-bold text-cream mb-2">{field.label}</div>
                  ...
                  <>
                    <div
                      ...
                      className="mx-journal-mode-selector mb-5 grid grid-cols-3 gap-1 rounded-[20px] border border-cream/10 bg-emerald p-1"
                    >
                      {Object.values(JOURNAL_MODES).map(mode => {
                        const active = journalMode === mode.key
                        ...
                        return (
                          <button
                            ...
                            onClick={() => {
                              platform.haptic('light')
                              setJournalMode(mode.key)
                            }}
                            className={[
                              'min-h-[44px] rounded-[15px] border-0 px-2 text-[12px] font-semibold transition-colors',
                    ...
                    <p className="mx-journal-mode-description mb-4 text-left text-[13px] leading-relaxed text-muted">
                      {journalModeConfig.description}
                    </p>
                    ...
                    <JournalTextarea
                      value={note}
                      onChange={setNote}
                      placeholder={journalModeConfig.placeholder}
