...
export default function CheckIn({
  ...
  const [morningDraft, setMorningDraft] =
    useState(() =>
      isEvening
        ? null
        : readCheckinDraft({ userId: user.id }),
    )

  const [draftStatus, setDraftStatus] =
    useState(() =>
      !isEvening && draftHasContent(morningDraft)
        ? 'restored'
        : 'idle',
    )
  ...
  const [savedMorningNote, setSavedMorningNote] =
    useState('')
  ...
  const note = isEvening
    ? ''
    : morningDraftToNote(morningDraft)

  const morningWritingMode =
    morningDraft?.mode || 'brief'
      ...
      {closeConfirmationOpen && (
        ...
        >
          <div className="w-full max-w-md mx-auto rounded-[28px] bg-emerald p-6 shadow-xl animate-fade-in">
            ...
            <p id="checkin-draft-dialog-description" className="mt-3 text-[14px] leading-relaxed text-muted">
              Есть несохранённая запись. Черновик останется только на этом устройстве и не будет выдан за сохранённую запись.
            </p>
