  ...
  return (
    <Shell onBack={onBack} onExit={onExit} progress={75} label="Первый шаг · 3 / 3">
      <section className="mx-morning__screen" aria-labelledby="morning-step-title">
        ...
        <p className="mx-morning__hint">
          Это не весь план. Только действие, с которого можно начать.
        </p>
      </section>
...
export default function MorningCheckinExperiment({ onComplete } = {}) {
  ...
  if (screen === 'today')
    return <TodayScreen main={selectedMain} firstStep={firstStep} onReset={reset} />
  if (screen === 'state') {
    return (
  ...
  return (
    <ResultScreen
      ...
      onConfirm={() => {
        if (onComplete) {
          onComplete(selectedMain, firstStep.trim())
        } else {
          setScreen('today')
        }
      }}
